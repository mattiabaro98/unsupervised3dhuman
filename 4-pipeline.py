...
def pipeline(front_path, back_path, gender, height, result_path):
    ...
    front_path_preprocessed = result_path + "front.ply"
    back_path_preproceessed = result_path + "back.ply"
    ...
    fitter.save_smpl_ply(pred_pose_front, pred_betas, pred_cam_trans_front + front_center_trans, result_path + "front_pred.ply")
    fitter.save_smpl_ply(pred_pose_back, pred_betas, pred_cam_trans_back + back_center_trans, result_path + "back_pred.ply")
...
for anonimo in anonimos:
    ...
    for sample in samples:
        ...
        result = pipeline(
            front_path, back_path, ground_truth_dict[anonimo]["gender"], float(ground_truth_dict[anonimo]["height"]), result_path
        )
